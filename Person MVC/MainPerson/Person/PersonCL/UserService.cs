...
namespace PService
{
    public class UserService : IUserService
    {
        ...
        public UserViewModel GetById(string id)
        {
            ...
            for (int i=0; i<user.people.Count; i++)
            {
                userVM.people[i].Address = new List<AddressViewModel>();
                foreach (var add in user.people[i].addresses)
                {
                    userVM.people[i].Address.Add(Mapper.Map<AddressViewModel>(add));
                }
            }
            return userVM;
        }
        ...
        public void AddPhoneNumber(PersonViewModel person)
        {
            PhoneNumbers pn = Mapper.Map<PhoneNumbers>(person.PhoneNumbers[person.PhoneNumbers.Count - 1]);
            Person personR = ToPerson(GetPersonById(person.Id));
            personR.phonenumbers.Add(pn);
            personR.phonenumbers[personR.phonenumbers.Count - 1].person=personR;
            repositoryPerson.Update(personR);
        }
        ...
        public List<CityViewModel> GetCities(int CountryID)
        {
            ...
            {
                citiesVM.Add(Mapper.Map<CityViewModel>(c));
            }
            return citiesVM;
        }
        ...
        public List<CountryViewModel> GetCountries()
        {
            ...
            List<CountryViewModel> countriesVM = Mapper.Map<List<CountryViewModel>>(counties);
            countriesVM = new List<CountryViewModel>();
        ...
        {
            return Mapper.Map<CityViewModel>(repositoryAddress.GetCityByID(id));
        }

        public CountryViewModel GetCountryByID(int id)
        {
            return Mapper.Map<CountryViewModel>(repositoryAddress.GetCouuntryById(id));
        }
    }
}
